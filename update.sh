...
EOF
    ...
    docker pull hotio/dotnetcore:stable-linux-arm64
    docker pull hotio/dotnetcore:stable-linux-arm
    docker pull hotio/dotnetcore:stable-linux-amd64
    docker inspect --format='{{index .RepoDigests 0}}' hotio/dotnetcore:stable-linux-arm64 >  upstream_digests.txt
    docker inspect --format='{{index .RepoDigests 0}}' hotio/dotnetcore:stable-linux-arm   >> upstream_digests.txt
    docker inspect --format='{{index .RepoDigests 0}}' hotio/dotnetcore:stable-linux-amd64 >> upstream_digests.txt
else
